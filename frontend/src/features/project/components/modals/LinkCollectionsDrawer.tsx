/**
 * LinkCollectionsDrawer - 关联集合抽屉组件
 */
...
interface LinkCollectionsDrawerProps {
    open: boolean
...
export function LinkCollectionsDrawer({ open, projectId, onClose, onSuccess }: LinkCollectionsDrawerProps) {
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const res = await projectsApi.getCollectionLinkOptions(projectId!)
    ...
    const handleSave = async () => {
        ...
        setSaving(true)
        try {
            const res = await projectsApi.updateProjectCollections(projectId, selectedIds)
            if (isSuccessfulDrawerResponse(res.code, res.message)) {
                message.success(res.message || "Links updated successfully")
