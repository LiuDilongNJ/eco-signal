...
Use the Users and permissions controls to assign a path role at a project or project-collection path. Global **Administrator** and **User** are system identities; Viewer, Annotator, Reviewer, Manager, and Custom apply only to the selected path. The same collection linked under another project is a separate path.

| Path role | Access |
| --- | --- |
| Viewer | Reads all media, sites, annotations, and reviews in the path |
| Annotator | Reads media and sites; creates and maintains only their own annotations |
| Reviewer | Reads all annotations and reviews; creates and maintains only their own reviews |
| Manager | Full access in the assigned project or collection path |
| Custom | Explicit permissions selected by a manager |

Select **Custom** before editing individual permissions. For annotations and reviews, **Own** applies only to objects created by that user; **All** applies to every object in the path. Project access is inherited by each linked collection and collection settings can add permissions but cannot reduce inherited access. `public_access` and `public_tags` remain read-only; public tags never publish reviews.

Runtime authorization uses the effective permissions for the current project-collection path and evaluates a single business action. Object responses expose resource-specific capabilities generated by the same policy used by the write endpoint, so a disabled action and a server-side denial follow the same rule. Missing capabilities are denied by default.
