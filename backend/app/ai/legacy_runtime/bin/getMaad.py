...
def load_audio(filename, channel):
    if Path(filename).suffix.lower() == '.wav':
        return maad.sound.load(filename, channel=channel)

    try:
        s, fs = sf.read(filename, always_2d=False)
    except Exception as exc:
        raise RuntimeError(f"unsupported or unreadable audio format: {filename}") from exc

    if getattr(s, "ndim", 1) == 2:
        if channel == 'left':
            s = s[:, 0]
        elif channel == 'right':
            s = s[:, 1] if s.shape[1] > 1 else s[:, 0]
        else:
            s = numpy.mean(s, axis=1)

    return s - numpy.mean(s), fs
...
def getMaad(filename, index_type, param, channel, minTime, maxTime, minFrequency, maxFrequency):
    ...
    if index_type == "acoustic_complexity_index":
        ...
        if param != '' and param is not None:
            for p in param.split('@'):
                parameter[p.split('?')[0]] = p.split('?')[1]
        s_wav, fs_wav = load_audio(filename, channel)
        peak_th = float(parameter['peak_th'])
        peak_distance = None if parameter['peak_distance'] in (None, '', 'None') else float(parameter['peak_distance'])
        # zoom
        ...
        if hasattr(rois, "to_json"):
            matches = rois.to_json(orient="records")
        elif isinstance(rois, (list, tuple, dict)):
            matches = json.dumps(rois)
        else:
            matches = "[]"
